...
def build_graph(train_data, triplets):
    ...
    for h_id, r_id, t_id in tqdm(triplets, ascii=True):
        ...
        # edges_type.append(r_id)
        rd[r_id].append([h_id, t_id])
        kgd[h_id].append((t_id, r_id))

    return ckg_graph, rd, kgd
...
def build_sparse_relational_graph(relation_dict):
    ...
    adj_mat_list[0] = adj_mat_list[0].tocsr()[:n_users, n_users:].tocoo()

    return adj_mat_list, norm_mat_list, mean_mat_list

def generate_ure_mat(kg_dict):
    def _si_norm_lap(adj):
        # D^{-1}A
        rowsum = np.array(adj.sum(1))
    ...
    for u in range(n_users):
        pos_ids = train_user_set[u]
        for i in pos_ids:
            es, rs = zip(*kg_dict[i])
            rows += [u * (n_relations-1) + (r-1) for r in rs]
            cols += list(es)
    
    vals = [1.] * len(rows)
    adj = sp.csr_matrix((vals, (rows, cols)), shape=(n_users*(n_relations-1), n_entities))
    adj = _si_norm_lap(adj)
    return adj

def build_graph_ue(kg_dict):
    graph_ue = nx.MultiDiGraph()
    for u_id in range(n_users):
        pos_ids = train_user_set[u_id]
        for i in pos_ids:
            for e_id, r_id in kg_dict[i]:
                graph_ue.add_edge(u_id, e_id, key=r_id, item=i)
    
    return graph_ue

def generate_re_adj(relation_dict):
    def _si_norm_lap(adj):
        # D^{-1}A
        rowsum = np.array(adj.sum(1))
    ...
    rels, entities = [], []
    for r in range(n_relations // 2):
        e1, e2 = zip(*relation_dict[r+1])
        rels += [r] * (len(e1) * 2)
        entities += e1 + e2
    
    vals = [1.] * len(rels)
    adj = sp.coo_matrix((vals, (rels, entities)), shape=(n_relations // 2, n_entities))
    adj = _si_norm_lap(adj)
    return adj

def generate_ue_adj(kg_dict):
    def _si_norm_lap(adj):
        # D^{-1}A
        rowsum = np.array(adj.sum(1))
    ...
    for u in range(n_users):
        pos_ids = train_user_set[u]
        for i in pos_ids:
            e_list, _ = zip(*kg_dict[i])
            users += [u] * len(e_list)
            entities += e_list
    
    vals = [1.] * len(users)
    adj = sp.coo_matrix((vals, (users, entities)), shape=(n_users, n_entities))
    adj = _si_norm_lap(adj)
...
def load_data(model_args):
    ...
    interaction = train_cf[train_cf[:,2] == 1][:,:2]
    ...
    return train_cf, eval_cf, user_dict, n_params, graph, \
           [adj_mat_list, norm_mat_list, mean_mat_list], interaction
